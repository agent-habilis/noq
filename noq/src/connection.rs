...
impl Drop for Connecting {
    fn drop(&mut self) {
        // If the handshake never completed, the connection task is still alive
        // (no idle/handshake timeout applies pre-handshake) — close it so the
        // endpoint reclaims it instead of leaking a half-open connection per
        // aborted handshake under churn. From n0-computer/noq#682.
